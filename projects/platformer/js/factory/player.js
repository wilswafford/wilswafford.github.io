    ...
    opspark.createPlayer = function (game) {
        ...
        function flyingJump() {
            let 
                ...
                origXOffset = asset.body.offset.x;
                console.log(`total frames: ${flyingJump.frameTotal}`);
                console.log(`origYOffset : ${origXOffset}`);
            
            asset.body.offset.x += 30 * _direction;
            asset.body.offset.y -= 0;
            asset.body.y -= 0;
            let onUpdate = function (anim, frame) {
                console.log(frame.index);
                if (frame.index < 52) {
                    console.log(`up y offset: ${asset.body.offset.y}`);
                    asset.body.offset.x -= 2;
                } else {
                    console.log(`down y offset: ${asset.body.offset.y}`);
                    asset.body.offset.x -= 2 * _direction;
